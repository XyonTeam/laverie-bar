 ...
 const user = location.state
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      let { data: caisse, error1 } = await supabase
      .from('caisse')
      .select('montant');
      ...
      if (caisse.length > 0) {
       ...
       let sommeRetrait = caisse.reduce((acc, item) => acc + item.montant, 0);
  ...
  async function AllUsers() {
    ...
    return (
      ...
      <div className='bg-[#042940] h-screen'>
        ...
        <div className='text-center text-4xl text-[#D6D58E]'>Les laveurs ont deja retire {retrait} F CFA</div>
            ...
            <li key={item.id} className="flex justify-between gap-x-6 py-5">
              ...
              <UserIcon className="h-6 w-6 text-gray-500" />
                <div className="min-w-0 flex-auto">
                  <p className="text-sm font-semibold leading-6 text-gray-900">{item.utilisateur.nom}</p>
                  <p className="mt-1 truncate text-xs leading-5 text-gray-500">Jour: {item.pris}</p>
                  <p className="mt-1 truncate text-xs leading-5 text-gray-500">Somme: {item.montant} F CFA</p>
                </div>
              </div>
